...
@app.route('/')
def index():
    recent_questions = get_recent_questions(5)  # 獲取最近5個問題
    return render_template('index.html', recent_questions=recent_questions, username=session.get('username'))


# 初始化RAGService
rag_service = RAGService(api_key="AIzaSyAt55QVbUbXcFqNGFfxspVrlgW3qKOLOOo")  # 替換為您的Google API Key
...
def ai_chat():
    ...
    return render_template('index.html')  # 返回首頁
# 其他路由保持不變...
